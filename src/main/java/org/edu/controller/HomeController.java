...
public class HomeController {
	...
	/**
	 * 루트 / 라는 이름으로 요쳥 받으면, home.jsp파일에 렌더링(화면출력) 됩니다.(아래)
	 ...
	 * 그래서, .java 클래스는 수정 후 약간 기다린 후 Reloading... 메시지 후 결과확인이 가능합니다.
	 */
	@RequestMapping(value="/contact",method=RequestMethod.POST)//포스트방식은 데이터를 전송하는 역할
	...
	public String index() {//메서드(함수)의 리턴(반환값출력)형태지정 메서드명(매개변수) {구현내용} String index(){}
		...
		//메서드명 제일 앞에있는 public는 접근권한에 대한 키워드인데요, public(공공-단른 클래스에서 접근가능)
		//, private(내부-현재 HoneController클래스에서만 접근가능)
		// public String index() { } 자바(스프링) 메서드 기본형태
		// $(document).ready(function(){ }; j쿼리 기본형태
		//리텅반환값으로 사용한 String 클래스는 문자열을 반환하고, 여기서는 index.jsp와 매칭되는 문자열을 반환.
	...
	public String home(Locale locale, Model model) {
		...
		System.out.println("함수-C언어나 자바스크립트-와 매서드-와,스프링에서 사용함-는 같은 대상을 가리킵니다");
		System.out.println("함수-메서드는 함수명('매개변수-입력값'){구현내용}형식이고, 입력값->출력값 구현됩니다.");
		Date date = new Date();//Data 날짜 관련 클래스형 변수 date 선언했습니다. date변수 메서를 사옹 가능하게됨.
		...
		model.addAttribute("TomcatserverTime", formattedDate );//derverTime 변수값으로 jsp파일로 이동.
		...
		//스프링에서 부단으로 데이터 이동(전송)에 대해서 살펴보았습니다.
	}*/
